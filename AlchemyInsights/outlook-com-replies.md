---
title: 9000240 Outlook.com отговори
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.custom:
- "1825"
- "9000240"
ms.openlocfilehash: 0fece3194ab9a14b8fb21123b0b92c2a9aabba62
ms.sourcegitcommit: 631cbb5f03e5371f0995e976536d24e9d13746c3
ms.translationtype: MT
ms.contentlocale: bg-BG
ms.lasthandoff: 04/22/2020
ms.locfileid: "43743766"
---
# <a name="replying-in-outlookcom"></a>Отговор в Outlook.com

За да отговорите на едно съобщение наведнъж:

1. В списъка със съобщения изберете съобщението, на което искате да отговорите.
2. Горе вдясно на екрана със съобщения изберете <img src='data:image/png;base64,' /> **Отговор** или <img src='data:image/png;base64,' /> **Отговорете на всички**.
3. Въведете съобщението си и след това изберете **Изпращане**.

За да отговорите автоматично на всички съобщения:

1. Отиди на <img src='data:image/png;base64,' />
**Настройки** > **Преглед на всички Outlook Настройки** > **Mail** > **Автоматични отговори,** за да отворите [настройките за автоматични отговори](https://outlook.live.com/mail/options/mail/automaticReplies).
2. Изберете превключвателя **Включване на автоматичните отговори**.
3. Отметване на изпращане на **отговори само за период от време** ви позволява да:
    - Изпращайте отговори само когато изберете. Ако не зададете период от време, автоматичният ви отговор остава включен, докато не го изключите.
    - Блокиране на календара ви
    - Автоматично отхвърляне на нови покани
    - Отклонявай и отменяй моите събрания през този период
4. В полето за съобщения въведете съобщение, което да изпратите на хората по време на престоя ви.
5. За да изпратите отговори само на вашите контакти, поставете отметка в квадратчето **Изпращане на отговори само на контактите.**
6. Изберете **Записване**.

Научете повече за [автоматичното препращане в Outlook.com](https://support.office.com/article/14614626-9855-48dc-a986-dec81d07b1a0?wt.mc_id=Office_Outlook_com_Alchemy).