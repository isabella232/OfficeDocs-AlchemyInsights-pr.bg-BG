---
title: Outlook.com с контакти
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ms.custom:
- "1850"
- "9000258"
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.openlocfilehash: a2bb861c4e8bc158b49138816553a37ff72ff8f0079cd6fdf98536697c857715
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: bg-BG
ms.lasthandoff: 08/05/2021
ms.locfileid: "54111929"
---
# <a name="organizing-your-outlookcom-mailbox"></a>Организиране на вашата Outlook.com пощенска кутия

Списъкът с контакти е колекция от имейл адреси и е полезен за изпращане на имейл до група хора. За да създадете списък с контакти:

1. Отидете в ["Хора"](https://outlook.live.com/people/) <img src='data:image/png;base64,' />.
1. В горния край на страницата изберете стрелката <img src='data:image/png;base64,' /> до Нов **контакт и** след това изберете Нов списък **с контакти**.
1. Въведете име за списъка и след това добавете имена или имейл адреси.
1. Изберете **Създаване**.

Научете повече за това [как да създавате, преглеждате и редактирате контакти и списъци с контакти в Outlook.com](https://support.office.com/article/5b909158-036e-4820-92f7-2a27f57b9f01?wt.mc_id=Office_Outlook_com_Alchemy).