---
title: Тестване на инхимията вградени изображения
ms.author: pebaum
author: pebaum
ms.date: 11/16/2017
ms.audience: ITPro
ms.topic: article
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.assetid: 28d8036f-9df2-4dff-835b-9424bfe988a2
ms.custom:
- "273"
- "1500006"
ms.openlocfilehash: 8a9d98e2af3ef0cc71b820b4e74aa2a2a522346b
ms.sourcegitcommit: 037331d71f06750d972c0b6278b23bb15c4806ca
ms.translationtype: MT
ms.contentlocale: bg-BG
ms.lasthandoff: 10/18/2019
ms.locfileid: "36743118"
---
# <a name="access-the-domains-page"></a>Достъп до страницата на домейните

За достъп до страницата на домейните, просто кликнете върху тази икона по-долу:

  
<img src='data:image/png;base64,'/>

След това можете да добавяте, модифицирате или премахвате домейна си.
За повече информация вижте [често задавани въпроси за домейни.](https://docs.microsoft.com/office365/admin/setup/domains-faq)
  