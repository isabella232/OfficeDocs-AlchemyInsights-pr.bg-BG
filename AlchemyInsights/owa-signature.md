---
title: Създаване на подпис в Outlook в уеб
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.custom: ''
ms.openlocfilehash: 550b1025213ae54c17aed9ae5829896d2c83a6196b3251bbd80a36e14e790b45
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: bg-BG
ms.lasthandoff: 08/05/2021
ms.locfileid: "54040927"
---
# <a name="create-an-email-signature-in-outlook-on-the-web"></a>Създаване на имейл подпис в Outlook в уеб

1. Отиди на <img src='data:image/png;base64,' />
 **Настройки**  >  **Преглед на всички Outlook настройки**  >  **Съставяне и отговор**.
1. Под **Имейл подпис въведете** подписа си и използвайте наличните опции за форматиране, за да промените облика му.

*Забележка:* Можете да имате само по един подпис на акаунт.

За да научите повече, [вижте Създаване и добавяне на имейл подпис в Outlook в уеб](https://support.office.com/article/5ff9dcfd-d3f1-447b-b2e9-39f91b074ea3).
