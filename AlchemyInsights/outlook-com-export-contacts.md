---
title: Експортиране на контакти в Outlook.com
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "279"
- "8000020"
ms.assetid: 37fc1455-7324-4c76-9a94-085a45e94c51
ms.openlocfilehash: 9f8a3c2ad2b2d6736d34f168bc5733d554ee32e95d8ea6f12069feea1a12ec49
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: bg-BG
ms.lasthandoff: 08/05/2021
ms.locfileid: "54024663"
---
# <a name="export-your-contacts-in-outlookcom"></a>Експортиране на контактите ви в Outlook.com

1. Отидете в ["Хора"](https://outlook.live.com/people/) <img src='data:image/png;base64,' />
.
2. В горния край на страницата изберете Управление **на** \> **експортирането**.
3. Изберете дали искате да експортирате всички контакти или само контакти от определена папка, и изберете **Експортиране**.
4. В долната част на страницата изберете Запиши **и** "contacts.csv" ще бъдат записани във вашата папка "Изтегляния" по подразбиране. За да изберете къде да запишете .csv файл, изберете стрелката до Запиши и изберете **Запиши като**.

За повече информация вижте [Експортиране на контакти от Outlook.com в CSV файл](https://support.office.com/article/578cca22-3550-4c73-b3f0-9978cfeac83f?wt.mc_id=Office_Outlook_com_Alchemy).