---
title: '8000019: Как да добавяте или импортирате контакти'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000019"
- "2002"
ms.assetid: ''
ms.openlocfilehash: 8e9535da046f21cc35ac9704492d8cd04e770aa997a6995f00dd3953241e471b
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: bg-BG
ms.lasthandoff: 08/05/2021
ms.locfileid: "53999769"
---
# <a name="import-contacts"></a>Импортиране на контакти

Преди да импортирате контакти в Outlook в уеб, ще трябва да ги експортирате от другия си доставчик на имейл като .csv файл. След това следвайте инструкциите по-долу.

1. В долната част на страницата изберете <img src='data:image/png;base64,' />
 **Хора**.
2. В лентата с инструменти изберете Управление **на**  >  **импортирането на контакти**.
3. Изберете **Преглед**, изберете вашия CSV файл и след това изберете **Отвори**.
4. Изберете **Импортиране**.

За да научите повече, вижте [Импортиране на контакти в Outlook](https://support.office.com/article/bb796340-b58a-46c1-90c7-b549b8f3c5f8#ID0EAACAAA=Outlook_on_the_web).

